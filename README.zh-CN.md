...
>
> 1.0.3 更新：
> - 修复切换账号和 `Refresh All` 时，复用额度 RPC 通道仍可能触发的闪退问题。
> - 超时后的通道读取现在会正确取消，不再走不安全的 `Process.terminationStatus` 访问路径。
> - 当 `app-server` 先结束输出、子进程尚未完全退出时，已保存账号批量刷新路径会稳定失败返回，不再导致程序崩溃。
