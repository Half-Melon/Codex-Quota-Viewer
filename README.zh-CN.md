...
>
> 1.0.2 更新：
> - 已保存账号在打开菜单时改为选择性刷新，只补刷 stale、expired、未缓存和暂态失败的账号。
> - 批量额度刷新改为分级超时，并对暂态 `app-server` 失败执行一次重试。
> - 已保存账号的额度读取改为复用短生命周期 RPC 通道，不再为每个账号都冷启动一个 `app-server`。
> - 修复了菜单打开时，复用 RPC 通道失效与 pipe 读取并发竞态导致的闪退。
